...
class LongDocFACTScore():
    def __init__(self, device="cuda", model="BARTScore"):
    ...
    def score_src_hyp_long(self, srcs, hyps):
        ...
        for src_idx, (src, hyp) in enumerate(zip(srcs, hyps)):
            logger.info(f"Scoring summary {src_idx + 1} of {len(srcs)}")
...
class BARTScore():
    def __init__(self, device="cuda", checkpoint="facebook/bart-large"):
        # Set up model
        self.device = device
    ...
    def bart_score(self, srcs, tgts, batch_size=4):
        ...
        for i in range(0, len(srcs), batch_size):
            ...
            try:
                ...
                traceback.print_exc()
                logger.error(f"Error processing batch: {src_list} -> {tgt_list}")
