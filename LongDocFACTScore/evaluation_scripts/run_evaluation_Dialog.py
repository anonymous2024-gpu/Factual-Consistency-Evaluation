...
parser.add_argument("--dataset", default="DialogSum", help="Dataset to run evaluation on")
args = parser.parse_args()

# Load the dataset
def load_dataframes(dataset):
    df = pd.read_csv('./data/DialogSum/DialogSum.csv')
    return df
...
def get_scores_from_metrics(df, src_cols, ref_col, document):
    ...
    df = update_df_with_metric_scores(df, get_ldfacts_row, src_cols, "ldfacts_ref_hyp", ref_col, switch_tgt=True)
    # df = update_df_with_metric_scores(df, get_ldfacts_row, ref_col, "ldfacts_doc_hyp", document, switch_tgt=True)
...
if __name__ == "__main__":
    ...
    output_path = f"./data/DialogSum/{args.dataset}_longdocfactscore.csv"
