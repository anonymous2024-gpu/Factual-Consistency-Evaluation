...
class DocDB(object):
    ...
    """
    def __init__(self, db_path=None, data_path=None,max_passage_length=256):
                ...
                for sent_idx, sent in enumerate(text):
                    ...
                    if len(tokens) <= max_length:
                        ...
                        while offset < len(tokens):
                            passages.append(tokens[offset:offset+self.max_passage_length])
    ...
    # def get_text_from_title(self, title):
    #     """Fetch the raw text of the doc for 'title'."""
    #     cursor = self.connection.cursor()
    #     cursor.execute("SELECT text FROM documents WHERE title = ?", (title,))
    #     results = cursor.fetchall()
    #     results = [r for r in results]
    ...
    #         logging.error(f"Title '{title}' not found in the database.")
    #         return []  # Or handle it according to your needs

    #     if len(results) > 1:
    #         logging.warning(f"Multiple entries found for title '{title}'. Using the first entry.")
        
    #     text = results[0][0]
    #     if not text:
    #         logging.error(f"No text found for title '{title}' in the database.")
    #         return []

    #     results = [{"title": title, "text": para} for para in text.split(self.SPECIAL_SEPARATOR)]

    #     if len(results) == 0:
    #         logging.error(f"No valid paragraphs found for title '{title}' after splitting.")
    #         return []
...
class Retrieval(object):
    ...
    def get_gtr_passages(self, topic, retrieval_query, passages, k): # returns back top k passages from wiki that are most similar to the query
