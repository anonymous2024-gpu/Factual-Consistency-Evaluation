...
class FactScorer(object):
    ...
    def get_score(self,
        ...
        init_scores = np.nan_to_num(init_scores, nan=0.0)

        out = {"score": np.mean(scores) if len(scores)>0 else 0.0,
               "respond_ratio": respond_ratio,
               "decisions": decisions,
               "topics": topics,
               "num_facts_per_response": np.mean([len(d) for d in decisions if d is not None]) if decisions else 0.0}

        if gamma:
            out["init_score"] = np.mean(init_scores) if len(init_scores) > 0 else 0.0
    ...
    def _get_score(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None,batch_size=4,k=5):
        ...
        for atom in atomic_facts:
            ...
            if self.lm:
                ...
                decisions.append({"atom": atom, "is_supported": True})
...
if __name__ == '__main__':
    ...
    with open(args.input_path) as f:
        data = list(f)  # Load all data into a list

        batch_size = 4 if "llama" in args.model_name else len(data)
        ...
        for i in range(0, len(data), batch_size):
            ...
            for line in batch:
                ...
                total_respond_ratio += out["respond_ratio"]
                total_num_facts_per_response += out["num_facts_per_response"]
                total_init_score += out["init_score"]
    ...
    if tot > 0:
        average_score = total_score / tot
        average_respond_ratio = total_respond_ratio / tot
        average_num_facts_per_response = total_num_facts_per_response / tot
        average_init_score = total_init_score / tot
        ...
        logging.critical("No entries processed.\n")
